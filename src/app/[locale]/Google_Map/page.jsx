...
export default function page({params}) {
    ...
    const t3 = useTranslations('teamDetails');
    const t4 = useTranslations('contact');
    const {locale} = params;
    return (
        <section dir={locale === "ar" ? "rtl" : "ltr"}>
            ...
            <div className='relative'>
                ...
                <div className='absolute w-full bottom-0 lg:p-20 md:p-10 p-5 flex flex-col md:flex-row md:justify-between items-start  md:items-center'>
                    <div>
                        <p className='text-green-600 font-bold'>// {t4("WelcomeToOurCompany")}</p>
                        <p className='text-white font-bold lg:text-6xl md:text-4xl text-2xl'>{t("googleMap")}</p>
                    </div>
                    <div className='flex gap-4'><Link href='/' className='text-white font-bold'>{t4("Home")}</Link><span className='text-white'>|</span><p className='text-green-600 font-bold'>{t3("Location")}</p></div>
                </div>
            </div>
