import { Blogs } from "../clientPage";
import { FiPhoneCall } from "react-icons/fi";
import { CiMail } from "react-icons/ci";
import { BsTwitterX, BsInstagram } from "react-icons/bs";
import {
    FaLocationArrow,
    FaFacebookF,
    FaTiktok,
    FaCheck,
} from "react-icons/fa";
import { IoLocationOutline } from "react-icons/io5";
import NavBarPages from "../components/NavbarPages";

import Link from "next/link";

import { useTranslations } from "next-intl";
export default function page({ params }) {
    const { locale } = params;
    const t2 = useTranslations("Home");
    const t = useTranslations("services");
    return (
        <section dir={locale === "ar" ? "rtl" : "ltr"}>
             
            <NavBarPages navLink="/" />
            <div className="relative">
                <img
                    src="/vegetables.jpg"
                    alt="image"
                    className="z-0 md:h-[600px] h-[400px] w-full"
                />
                <div className="absolute w-full bottom-0 lg:p-20 md:p-10 p-5 flex flex-col md:flex-row md:justify-between items-start  md:items-center">
                    <div>
                        <p className="text-green-600 font-bold">
                            // {t("WelcomeToOurCompany")}
                        </p>
                        <p className="text-white font-bold lg:text-6xl md:text-4xl text-2xl">
                            {t("WhatWeDo")}
                        </p>
                    </div>
                    <div className="flex gap-4">
                        <Link href="/" className="text-white font-bold">
                            {t("Home")}
                        </Link>
                        <span className="text-white">|</span>
                        <p className="text-green-600 font-bold"> {t("Services")}</p>
                    </div>
                </div>
            </div>
            <section className="flex flex-col md:flex-row my-20 lg:px-20 md:px-10 px-4 items-center">
                <div className="md:flex hidden bg-green-600 w-15 h-90"></div>
                <img
                    src="https://broccolisite.netlify.app/assets/services1-DfXvJjdF.webp"
                    alt="image"
                    className="h-[450px]"
                />
                <div className={`ml-10 ${locale === "ar" ? "mr-4" : ""}`}>
                    
                    <p className="text-green-600 font-bold">
                        // {t("ReliableServices")}
                    </p>
                    <p className="lg:text-6xl md:text-4xl text-3xl font-extrabold">
                        {t("WeAreQualifiedProfessional")}
                        <span className="text-green-600">.</span>
                    </p>
                    <div className="flex flex-col md:flex-row">
                        <div className="md:w-9/12 w-full">
                            <p className={`after:content-[""] after:h-12 after:bg-green-600 after:w-1 after:absolute after:top-0  relative px-5 my-10 ${locale === "ar" ? "after:right-0" : "after:left-0"}`}>
                                {t("TopQualityService")}
                            </p>
                            <p>
                                {t("CommittedToExcellence")}
                            </p>
                        </div>
                        <div className="md:w-3/12 w-full flex flex-col gap-4 items-start">
                            <p className="flex gap-3 items-center">
                                <FaCheck className="text-green-600" />
                                {t("OnlineSupport")}
                            </p>
                            <p className="flex gap-3 items-center">
                                <FaCheck className="text-green-600" />
                                {t("ExpertTeam")}
                            </p>
                            <p className="flex gap-3 items-center">
                                <FaCheck className="text-green-600" />
                                {t("PureEquipment")}
                            </p>
                            <p className="flex gap-3 items-center">
                                <FaCheck className="text-green-600" />
                                {t("UnlimitedProduct")}
                            </p>
                        </div>
                    </div>
                </div>
            </section>
            <section>
                <h2 className="font-bold text-6xl text-center pb-10">
                    {t("OurServices")}
                </h2>
                <div className="flex flex-wrap gap-15 justify-center lg:px-20 md:px-10 px-4">
                    <div className="lg:w-3/12 md:w-6/12 w-fu groupll shadow">
                        <div className="overflow-hidden">
                            <img
                                className="group-hover:scale-110"
                                src="https://broccolisite.netlify.app/assets/ourservice1-Bm1_Wv1B.webp"
                                alt="image"
                            />
                        </div>
                        
                        <h2 className="p-2">{t("OrganicVegetables1")}</h2>
                        <p className="p-2">
                            {t("OrganicVegetablesDesc1")}
                        </p>
                    </div>
                    <div className="lg:w-3/12 md:6/12 w-full group shadow">
                        <div className="overflow-hidden">
                            <img
                                className="group-hover:scale-110"
                                src="https://broccolisite.netlify.app/assets/ourservice2-xmW1oWjT.webp"
                                alt="image"
                            />
                        </div>
                        <h2 className="p-2">{t("OrganicMeat1")}</h2>
                        <p className="p-2">
                            {t("OrganicMeatDesc1")}
                        </p>
                    </div>
                    <div className="lg:w-3/12 md:6/12 w-full group shadow">
                        <div className="overflow-hidden">
                            <img
                                className="group-hover:scale-110"
                                src="https://broccolisite.netlify.app/assets/ourservice3-iGcGE17j.webp"
                                alt="image"
                            />
                        </div>
                        <h2 className="p-2">{t("FirmFreshFruits1")}</h2>
                        <p className="p-2">
                            {t("FirmFreshFruitsDesc1")}
                        </p>
                    </div>
                    <div className="lg:w-3/12 md:6/12 w-full group shadow">
                        <div className="overflow-hidden">
                            <img
                                className="group-hover:scale-110"
                                src="https://broccolisite.netlify.app/assets/ourservice2-xmW1oWjT.webp"
                                alt="image"
                            />
                        </div>

                        <h2 className="p-2">{t("OrganicMeat2")}</h2>
                        <p className="p-2">
                            {t("OrganicMeatDesc2")}
                        </p>
                    </div>
                    <div className="lg:w-3/12 md:6/12 w-full group shadow">
                        <div className="overflow-hidden">
                            <img
                                className="group-hover:scale-110"
                                src="https://broccolisite.netlify.app/assets/ourservice1-Bm1_Wv1B.webp"
                                alt="image"
                            />
                        </div>
                        <h2 className="p-2">{t("OrganicVegetables2")}</h2>
                        <p className="p-2">
                            {t("OrganicVegetablesDesc2")}
                        </p>
                    </div>
                    <div className="lg:w-3/12 md:6/12 w-full group shadow">
                        <div className="overflow-hidden">
                            <img
                                className="group-hover:scale-110"
                                src="https://broccolisite.netlify.app/assets/ourservice3-iGcGE17j.webp"
                                alt="image"
                            />
                        </div>
                        <h2 className="p-2">{t("FirmFreshFruits2")}</h2>
                        <p className="p-2">
                            {t("FirmFreshFruitsDesc2")}
                        </p>
                    </div>
                </div>
            </section>
            <Blogs />
            <footer className="lg:mt-30 md:mt-20 mt-10 relative">
                <div className="flex flex-wrap lg:px-10 md:px-7 px-4 lg:absolute lg:-top-20">
                    <div className="flex items-center bg-white gap-5 w-full sm:w-6/12 lg:w-3/12 p-7 shadow-[0_0_1px_gray]">
                        <img
                            className="w-10 h-10"
                            src="data:image/webp;base64,"
                            alt="image"
                        />
                        <div>
                            <p className="font-bold">{t2("card1Title")}</p>
                            <p>{t2("card1Desc")}</p>
                        </div>
                    </div>
                    <div className="flex items-center bg-white gap-5 w-full sm:w-6/12 lg:w-3/12 p-7 shadow-[0_0_1px_gray]">
                        <img
                            className="w-10 h-10"
                            src="data:image/webp;base64,"
                            alt="image"
                        />
                        <div>
                            <p className="font-bold">{t2("card2Title")}</p>
                            <p>{t2("card2Desc")}</p>
                        </div>
                    </div>
                    <div className="flex items-center bg-white gap-5 w-full sm:w-6/12 lg:w-3/12 p-7 shadow-[0_0_1px_gray]">
                        <img
                            className="w-10 h-10"
                            src="data:image/webp;base64,"
                            alt="image"
                        />
                        <div>
                            <p className="font-bold">{t2("card3Title")}</p>
                            <p>{t2("card3Desc")}</p>
                        </div>
                    </div>
                    <div className="flex items-center bg-white gap-5 w-full sm:w-6/12 lg:w-3/12 p-7 shadow-[0_0_1px_gray]">
                        <img
                            className="w-10 h-10"
                            src="data:image/webp;base64,"
                            alt="image"
                        />
                        <div>
                            <p className="font-bold">{t2("card4Title")}</p>
                            <p>{t2("card4Desc")}</p>
                        </div>
                    </div>
                </div>
                <div className="flex flex-wrap gap-8 bg-[#F7F5EB] lg:px-10 md:px-7 px-4 lg:pt-30 pb-8">
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <img
                            className="w-8/12"
                            src="data:image/webp;base64,"
                            alt=""
                        />
                        <p>{t2("footerText")}</p>
                        <div className="flex gap-4 items-center">
                            <IoLocationOutline />
                            Brooklyn, New York, United States
                        </div>
                        <div className="flex gap-4 items-center">
                            <FiPhoneCall />
                            +0123-456789
                        </div>
                        <div className="flex gap-4 items-center">
                            <CiMail />
                            example@example.com
                        </div>
                        <div className="flex gap-2">
                            <FaFacebookF />
                            <FaTiktok />
                            <BsInstagram />
                            <BsTwitterX />
                        </div>
                    </div>
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <p className="font-bold">{t2("company")}</p>
                        <Link href="/About" className="hover:text-green-600">
                            {t2("about")}
                        </Link>
                        <Link href="/News" className="hover:text-green-600">
                            {t2("blog")}
                        </Link>
                        <Link href="/Shop" className="hover:text-green-600">
                            {t2("allProducts")}
                        </Link>
                        <Link
                            href="/Google_Map"
                            className="hover:text-green-600"
                        >
                            {t2("locationMap")}
                        </Link>
                        <Link href="/FAQ" className="hover:text-green-600">
                            {t2("faq")}
                        </Link>
                        <Link href="/Contact" className="hover:text-green-600">
                            {t2("contactUs")}
                        </Link>
                    </div>
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <p className="font-bold">{t2("services")}</p>
                        <Link
                            href="/Order_Tracking"
                            className="hover:text-green-600"
                        >
                            {t2("orderTracking")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("wishlist")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("login")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("myAccount")}
                        </Link>
                        <Link href="/About" className="hover:text-green-600">
                            {t2("termsConditions")}
                        </Link>
                        <Link href="/Shop" className="hover:text-green-600">
                            {t2("promotionalOffers")}
                        </Link>
                    </div>
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <p className="font-bold">{t2("customerCare")}</p>
                        <Link href="/" className="hover:text-green-600">
                            {t2("wishlist")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("login")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("myAccount")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("orderTracking")}
                        </Link>
                        <Link href="/FAQ" className="hover:text-green-600">
                            {t2("faq")}
                        </Link>
                        <Link href="/Contact" className="hover:text-green-600">
                            {t2("contactUs")}
                        </Link>
                    </div>
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <p className="font-bold">{t2("newsletter")}</p>
                        <p>{t2("subscribe")}</p>
                        <div className="flex">
                            <input
                                type="text"
                                placeholder={`${t2("email")}*`}
                                className="border-2 border-gray-50 outline-0 p-4 focus:border-green-600"
                            />
                            <button className="p-4 text-white bg-green-600">
                                <FaLocationArrow />
                            </button>
                        </div>
                        <p className="font-bold">{t2("weAccept")}</p>
                        <img
                            src="data:image/webp;base64,"
                            alt="visa image"
                        />
                    </div>
                </div>
                <div className="flex md:flex-row flex-col justify-around bg-black p-8">
                    <p className="text-white">{t2("rights")}</p>
                    <div className="flex gap-4 md:flex-row flex-col">
                        <a href="#" className="hover:text-green-500 text-white">
                            {t2("termsConditions")}
                        </a>
                        <a href="#" className="hover:text-green-500 text-white">
                            {t2("claim")}
                        </a>
                        <a href="#" className="hover:text-green-500 text-white">
                            {t2("privacyPolicy")}
                        </a>
                    </div>
                </div>
            </footer>
        </section>
    );
}
