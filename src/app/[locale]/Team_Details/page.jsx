import { FiPhoneCall } from "react-icons/fi";
import { CiMail } from "react-icons/ci";
import { BsTwitterX, BsInstagram } from "react-icons/bs";
import { FaLocationArrow, FaFacebookF, FaTiktok } from "react-icons/fa";
import { IoLocationOutline } from "react-icons/io5";

import NavBarPages from "../components/NavbarPages";
import Link from "next/link";

import { useTranslations } from "next-intl";

export default function page({ params }) {
    const t = useTranslations("teamDetails");
    const t2 = useTranslations("Home");
    const t3 = useTranslations("team");
    const locale = params.locale;
    return (
        <section dir={locale === "ar" ? "rtl" : "ltr"}>
            <NavBarPages navLink="/" />
            <div className="relative">
                <img
                    src="/vegetables.jpg"
                    alt="image"
                    className="z-0 md:h-[600px] h-[400px] w-full"
                />
                <div className="absolute w-full bottom-0 lg:p-20 md:p-10 p-5 flex flex-col md:flex-row md:justify-between items-start  md:items-center">
                    <div>
                        <p className="text-green-600 font-bold">
                            // {t3("WelcomeToOurCompany")}
                        </p>
                        <p className="text-white font-bold lg:text-6xl md:text-4xl text-2xl">
                            {t("TeamDetails")}
                        </p>
                    </div>
                    <div className="flex gap-4">
                        <Link href="/" className="text-white font-bold">
                            {t("Home")}
                        </Link>
                        <span className="text-white">|</span>
                        <p className="text-green-600 font-bold">{t("TeamDetails")}</p>
                    </div>
                </div>
            </div>

            <div className="lg:px-20 md:px-10 px-4 my-10 flex md:flex-row flex-col gap-10">
                <div className="md:w-4/12 w-full flex flex-col items-center gap-3">
                    <img
                        src="https://broccolisite.netlify.app/assets/adam-D8LrdnbM.webp"
                        alt="image"
                    />
                    <p className="font-bold">Rosalina D. William</p>
                    <p className="font-bold text-green-600">{t("Founder")}</p>
                    <div className="flex gap-2">
                        <FaFacebookF />
                        <FaTiktok />
                        <BsInstagram />
                        <BsTwitterX />
                    </div>
                </div>
                <div className="md:w-8/12 w-full">
                    <p>{t("FounderBio1")}</p>
                    <div className="my-10">
                        <div className="flex md:gap-5 ">
                            <p className="font-bold">{t("Positions")}:</p>
                            <p>{t("SeniorTechnician")}</p>
                        </div>
                        <div className="flex md:gap-5 ">
                            <p className="font-bold">{t("Experience")}:</p>
                            <p>{t("TenPlusYears")}</p>
                        </div>
                        <div className="flex md:gap-5 ">
                            <p className="font-bold">{t("Location")}:</p>
                            <p>{t("Address")}</p>
                        </div>
                        <div className="flex md:gap-5 ">
                            <p className="font-bold">{t("PracticeArea")}:</p>
                            <p>{t("CarRepair")}</p>
                        </div>
                        <div className="flex md:gap-5 ">
                            <p className="font-bold">{t("Email")}:</p>
                            <p>example@example.com</p>
                        </div>
                        <div className="flex md:gap-5 ">
                            <p className="font-bold">{t("Fax")}:</p>
                            <p>+0123-456 789</p>
                        </div>
                        <div className="flex md:gap-5 ">
                            <p className="font-bold">{t("Phone")}:</p>
                            <p>+0123-456 789</p>
                        </div>
                    </div>
                    <p>{t("FounderBio2")}</p>
                    <div className="flex md:flex-row flex-col gap-5 my-5">
                        <div className="flex justify-between flex-col gap-4 shadow lg:w-3/12 md:w-4/12 w-full p-3">
                            <img
                                className="h-15 w-15"
                                src="data:image/webp;base64,"
                                alt="image"
                            />
                            <p className="font-bold">{t("AllKindBrand")}</p>
                            <p>{t("AllKindBrandDesc")}</p>
                            <p className="font-bold hover:text-green-600 duration-500 cursor-pointer">
                                {t("ReadMore")}
                            </p>
                        </div>
                        <div className="flex justify-between flex-col gap-4 shadow lg:w-3/12 md:w-4/12 w-full p-3">
                            <img
                                className="h-15 w-15"
                                src="data:image/webp;base64,"
                                alt="image"
                            />
                            <p className="font-bold">{t("CuratedProducts")}</p>
                            <p>{t("CuratedProductsDesc")}</p>
                            <p className="font-bold hover:text-green-600 duration-500 cursor-pointer">
                                {t("ReadMore")}
                            </p>
                        </div>
                        <div className="flex justify-between flex-col gap-4 shadow lg:w-3/12 md:w-4/12 w-full p-3">
                            <img
                                className="h-15 w-15"
                                src="data:image/webp;base64,"
                                alt="image"
                            />
                            <p className="font-bold">
                                {t("PesticideFreeGoods")}
                            </p>
                            <p>{t("PesticideFreeGoodsDesc")}</p>
                            <p className="font-bold hover:text-green-600 duration-500 cursor-pointer">
                                {t("ReadMore")}
                            </p>
                        </div>
                    </div>
                    <p>{t("OurJourneyDesc")}</p>
                </div>
            </div>
            <footer className="lg:mt-30 relative">
                <div className="flex flex-wrap lg:px-10 md:px-7 px-4 lg:absolute lg:-top-20">
                    <div className="flex items-center bg-white gap-5 w-full sm:w-6/12 lg:w-3/12 p-7 shadow-[0_0_1px_gray]">
                        <img
                            className="w-10 h-10"
                            src="data:image/webp;base64,"
                            alt="image"
                        />
                        <div>
                            <p className="font-bold">{t2("card1Title")}</p>
                            <p>{t2("card1Desc")}</p>
                        </div>
                    </div>
                    <div className="flex items-center bg-white gap-5 w-full sm:w-6/12 lg:w-3/12 p-7 shadow-[0_0_1px_gray]">
                        <img
                            className="w-10 h-10"
                            src="data:image/webp;base64,"
                            alt="image"
                        />
                        <div>
                            <p className="font-bold">{t2("card2Title")}</p>
                            <p>{t2("card2Desc")}</p>
                        </div>
                    </div>
                    <div className="flex items-center bg-white gap-5 w-full sm:w-6/12 lg:w-3/12 p-7 shadow-[0_0_1px_gray]">
                        <img
                            className="w-10 h-10"
                            src="data:image/webp;base64,"
                            alt="image"
                        />
                        <div>
                            <p className="font-bold">{t2("card3Title")}</p>
                            <p>{t2("card3Desc")}</p>
                        </div>
                    </div>
                    <div className="flex items-center bg-white gap-5 w-full sm:w-6/12 lg:w-3/12 p-7 shadow-[0_0_1px_gray]">
                        <img
                            className="w-10 h-10"
                            src="data:image/webp;base64,"
                            alt="image"
                        />
                        <div>
                            <p className="font-bold">{t2("card4Title")}</p>
                            <p>{t2("card4Desc")}</p>
                        </div>
                    </div>
                </div>
                <div className="flex flex-wrap gap-8 bg-[#F7F5EB] lg:px-10 md:px-7 px-4 lg:pt-30 pb-8">
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <img
                            className="w-8/12"
                            src="data:image/webp;base64,"
                            alt=""
                        />
                        <p>{t2("footerText")}</p>
                        <div className="flex gap-4 items-center">
                            <IoLocationOutline />
                            Brooklyn, New York, United States
                        </div>
                        <div className="flex gap-4 items-center">
                            <FiPhoneCall />
                            +0123-456789
                        </div>
                        <div className="flex gap-4 items-center">
                            <CiMail />
                            example@example.com
                        </div>
                        <div className="flex gap-2">
                            <FaFacebookF />
                            <FaTiktok />
                            <BsInstagram />
                            <BsTwitterX />
                        </div>
                    </div>
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <p className="font-bold">{t2("company")}</p>
                        <Link href="/About" className="hover:text-green-600">
                            {t2("about")}
                        </Link>
                        <Link href="/News" className="hover:text-green-600">
                            {t2("blog")}
                        </Link>
                        <Link href="/Shop" className="hover:text-green-600">
                            {t2("allProducts")}
                        </Link>
                        <Link
                            href="/Google_Map"
                            className="hover:text-green-600"
                        >
                            {t2("locationMap")}
                        </Link>
                        <Link href="/FAQ" className="hover:text-green-600">
                            {t2("faq")}
                        </Link>
                        <Link href="/Contact" className="hover:text-green-600">
                            {t2("contactUs")}
                        </Link>
                    </div>
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <p className="font-bold">{t2("services")}</p>
                        <Link
                            href="/Order_Tracking"
                            className="hover:text-green-600"
                        >
                            {t2("orderTracking")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("wishlist")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("login")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("myAccount")}
                        </Link>
                        <Link href="/About" className="hover:text-green-600">
                            {t2("termsConditions")}
                        </Link>
                        <Link href="/Shop" className="hover:text-green-600">
                            {t2("promotionalOffers")}
                        </Link>
                    </div>
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <p className="font-bold">{t2("customerCare")}</p>
                        <Link href="/" className="hover:text-green-600">
                            {t2("wishlist")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("login")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("myAccount")}
                        </Link>
                        <Link href="/" className="hover:text-green-600">
                            {t2("orderTracking")}
                        </Link>
                        <Link href="/FAQ" className="hover:text-green-600">
                            {t2("faq")}
                        </Link>
                        <Link href="/Contact" className="hover:text-green-600">
                            {t2("contactUs")}
                        </Link>
                    </div>
                    <div className="flex flex-col gap-5 md:w-2/12">
                        <p className="font-bold">{t2("newsletter")}</p>
                        <p>{t2("subscribe")}</p>
                        <div className="flex">
                            <input
                                type="text"
                                placeholder={`${t2("email")}*`}
                                className="border-2 border-gray-50 outline-0 p-4 focus:border-green-600"
                            />
                            <button className="p-4 text-white bg-green-600">
                                <FaLocationArrow />
                            </button>
                        </div>
                        <p className="font-bold">{t2("weAccept")}</p>
                        <img
                            src="data:image/webp;base64,"
                            alt="visa image"
                        />
                    </div>
                </div>
                <div className="flex md:flex-row flex-col justify-around bg-black p-8">
                    <p className="text-white">{t2("rights")}</p>
                    <div className="flex gap-4 md:flex-row flex-col">
                        <a href="#" className="hover:text-green-500 text-white">
                            {t2("termsConditions")}
                        </a>
                        <a href="#" className="hover:text-green-500 text-white">
                            {t2("claim")}
                        </a>
                        <a href="#" className="hover:text-green-500 text-white">
                            {t2("privacyPolicy")}
                        </a>
                    </div>
                </div>
            </footer>
        </section>
    );
}
